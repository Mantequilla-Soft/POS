'use strict';

/**
 * POSHIVE Payment Plugin Manifest
 *
 * To add a new payment plugin:
 *   1. Add one entry to window.PAYMENT_PLUGINS below.
 *   2. If the plugin needs custom POS logic beyond a QR or notes prompt,
 *      add a case to handlePluginPayment() in pos.html.
 *   3. If the plugin needs a backend route, add it to backend/routes/sales.js.
 *   That's it — admin panel and POS buttons appear automatically.
 *
 * configField types: 'text' | 'password' | 'select' | 'textarea'
 * serverOnly: true  → field is saved to DB but never returned to the frontend.
 *                      The frontend will see `${key}_saved: true` instead.
 * storeRoot: true   → value is stored at store.hiveAccount (not in pluginConfigs).
 * handler:          → which POS handler to invoke (see handlePluginPayment in pos.html)
 *                      built-in: 'hive' | 'stripe' | 'lightning' | 'qr' | 'external'
 */

window.PAYMENT_PLUGINS = [

  // ── Hive / HBD ─────────────────────────────────────────────────────────────
  {
    id:          'hive',
    name:        'Hive / HBD',
    nameKey:     'plugin.hive.name',
    description: 'Accept HBD ($1 stable crypto) via QR code. Customers scan with any Hive wallet.',
    descKey:     'plugin.hive.desc',
    icon:        '🐝',
    color:       '#e31337',
    posLabel:    'pos.payHbd',
    handler:     'hive',
    configFields: [
      {
        key:         'hiveAccount',
        label:       'Hive Account (receives payments)',
        labelKey:    'plugin.hive.accountLabel',
        type:        'text',
        placeholder: 'yourhiveaccount',
        storeRoot:   true,
        help:        'This is your store\'s Hive account — also used for membership reminders.',
        helpKey:     'plugin.hive.accountHelp',
      },
    ],
  },

  // ── Stripe ─────────────────────────────────────────────────────────────────
  {
    id:          'stripe',
    name:        'Stripe Card Payments',
    nameKey:     'plugin.stripe.name',
    description: 'Accept credit and debit cards at the POS. Requires your own Stripe account (stripe.com).',
    descKey:     'plugin.stripe.desc',
    icon:        '💳',
    color:       '#635bff',
    posLabel:    'pos.payCard',
    handler:     'stripe',
    configFields: [
      {
        key:         'publishableKey',
        label:       'Publishable Key',
        labelKey:    'plugin.stripe.pubKeyLabel',
        type:        'text',
        placeholder: 'pk_live_…',
      },
      {
        key:        'secretKey',
        label:      'Secret Key',
        labelKey:   'plugin.stripe.secKeyLabel',
        type:       'password',
        placeholder:'sk_live_…',
        serverOnly: true,
        help:       'Stored securely on the server — never sent to the browser.',
        helpKey:    'plugin.stripe.secKeyHelp',
      },
    ],
  },

  // ── Bitcoin Lightning ───────────────────────────────────────────────────────
  {
    id:          'lightning',
    name:        'Bitcoin Lightning',
    nameKey:     'plugin.lightning.name',
    description: 'Accept BTC via Lightning Network using the v4v.app API.',
    descKey:     'plugin.lightning.desc',
    icon:        '⚡',
    color:       '#f7931a',
    posLabel:    'pos.payBtc',
    handler:     'lightning',
    configFields: [
      {
        key:         'hive_accname',
        label:       'Hive Account (for v4v.app)',
        labelKey:    'plugin.lightning.accountLabel',
        type:        'text',
        placeholder: 'yourhiveaccount',
      },
      {
        key:      'receive_currency',
        label:    'Receive payments as',
        labelKey: 'plugin.lightning.currencyLabel',
        type:     'select',
        options: [
          { value: 'hbd',  label: 'HBD'       },
          { value: 'hive', label: 'HIVE'       },
          { value: 'sats', label: 'Keep Sats'  },
        ],
      },
    ],
  },

  // ── Bank Transfer ───────────────────────────────────────────────────────────
  {
    id:          'bankTransfer',
    name:        'Bank / Wire Transfer',
    nameKey:     'plugin.bankTransfer.name',
    description: 'Show transfer instructions to customers at checkout. Works for any country — wire, SEPA, Pago Móvil, etc.',
    descKey:     'plugin.bankTransfer.desc',
    icon:        '🏦',
    color:       '#0ea5e9',
    noPosButton: true,   // uses the existing built-in Bank Transfer button
    configFields: [
      {
        key:         'instructions',
        label:       'Transfer Instructions',
        labelKey:    'plugin.bankTransfer.instrLabel',
        type:        'textarea',
        placeholder: 'Bank: Example Bank\nAccount Type: Savings\nAccount #: 1234567890\nAccount Holder: Your Name\nID: 0987654321',
        help:        'Shown to the cashier and customer when Bank Transfer is selected at checkout.',
        helpKey:     'plugin.bankTransfer.instrHelp',
      },
      {
        key:         'imageUrl',
        label:       'QR Code / Account Image (optional)',
        labelKey:    'plugin.bankTransfer.imageLabel',
        type:        'image-upload',
        placeholder: 'https://...',
        help:        'Upload a QR code, payment app image, or formatted account card.',
        helpKey:     'plugin.bankTransfer.imageHelp',
      },
    ],
  },

  // ── Add future plugins below ────────────────────────────────────────────────
  //
  // Example — USDT (TRC-20):
  // {
  //   id:          'usdt',
  //   name:        'USDT (TRC-20)',
  //   description: 'Accept Tether on the Tron network via QR code.',
  //   icon:        '💵',
  //   color:       '#26a17b',
  //   posLabel:    'pos.payUsdt',
  //   handler:     'qr',     // uses the generic QR display + manual confirm flow
  //   configFields: [
  //     { key: 'walletAddress', label: 'TRC-20 Wallet Address', type: 'text', placeholder: 'T...' },
  //   ],
  // },

];
